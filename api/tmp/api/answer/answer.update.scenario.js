...
scenario.step('update an answer', function(answer) { 
	return this.put({
    url: '/' + poll._id + '/participations/' + participation._id + '/answers/' + answer._id + '?question=' + question._id + '&choice=' + choices[1]._id,
    expect: {
      statusCode: 200
    }
  });
...
scenario.step('log update answer', function(response) {
	var answer = response.body;
  if (answer.choice !== choices[1]._id || answer.participation !== participation._id) {
    return this.fail('created answer does not match');
  }
  console.log(response.statusCode);
  console.log(answer);
